...
// These lay out their own scrolling region (and, for songs, its own header
// row) rather than sitting inside the shared scroll wrapper below.
const SELF_SCROLLING: string[] = ["collection", "ai", "songs"];

// The songs tab's multi-select doesn't make sense to reach from every tab.
const SHOW_IMPORT: string[] = ["playlists", "collection", "songs"];
...
function App() {
  ...
  const selectActive = useSongSelectionStore((s) => s.active);
  const selectedCount = useSongSelectionStore((s) => s.selected.size);
  ...
  // Leaving the songs tab with a selection in progress should not leave it
  // active for the next visit.
  useEffect(() => {
    if (tab !== "songs" && selectActive) clearSelection();
  }, [tab, selectActive, clearSelection]);
  ...
  const renderContent = () => {
    ...
    const songsSelecting = tab === "songs" && selectActive;
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between px-4 pb-2 shrink-0 safe-top">
          {songsSelecting ? (
            <>
              <h1 className="text-lg font-semibold">{selectedCount}개 선택됨</h1>
              <button
                type="button"
                onClick={clearSelection}
                className="flex items-center gap-1 text-nm-accent text-[14px] font-medium"
              >
            ...
            <>
              ...
              <div className="flex items-center gap-2">
                {SHOW_IMPORT.includes(tab) && <ImportButton />}
                <button
                  type="button"
                  aria-label="설정"
